...
forzar_exp_inter <= (bex_val and pet_fallo_exp) and pet_fallo;
conc_fallo_inter <= (pet_fallo and (not forzar_exp_inter)) and (not mem_ocupada);

forzar_exp <= forzar_exp_inter;
conc_fallo <= conc_fallo_inter;
conc_exp <= (pet_bex and (not conc_fallo_inter)) and (not mem_ocupada);
